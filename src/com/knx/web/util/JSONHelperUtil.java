...
public class JSONHelperUtil {
	...
	/**
	 * 清除掉List<Map>转�?json之�?�，字符串中出现的{"map":...},使之更容易被�?端解�?
	 */
	...
	/**
	 * 在异步获�?�列表�?�生错误时，返回一个outputDTOToJSON方法�?�用的erroMap
	 * 直接用outputOperationResultAsJSON返回的�?，�?�能会影�?到ext的grid或者jquery的datatable的内部代�?的�?行
	 */
...
//		System.out.println(multiDtoListToJSON(map));
		...
		List<AccountSchoolDTO> accountSchoolDTOList = new ArrayList<AccountSchoolDTO>();
		AccountSchoolDTO accountSchoolDTO = new AccountSchoolDTO();
		accountSchoolDTO.setDegree("degree");
		accountSchoolDTO.setEntrytime("entrytime");
		accountSchoolDTO.setId(1161L);
		accountSchoolDTO.setInstitute("");
		accountSchoolDTO.setName(null);
		
		accountSchoolDTOList.add(accountSchoolDTO);
		accountSchoolDTOList.add(accountSchoolDTO);
		//
		...
		accountServiceDTO.setAccountId(1161L);
		accountServiceDTO.setAccountPhotoUrl("accountPhotoUrl");
		accountServiceDTO.setTags(studentIdList);
		
		System.out.println(JSONHelperUtil.dtoToJSON(accountSchoolDTO));
		System.out.println(JSONHelperUtil.dtoToJSON(accountSchoolDTOList));
		System.out.println(JSONHelperUtil.dtoToJSON(accountServiceDTO));
		System.out.println(JSONHelperUtil.dtoToJSON("success"));
	}*/
}
